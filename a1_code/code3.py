...
def plot_training_curve(val_f1_history, filename="training_curve.png"):
    n = len(val_f1_history)
    epochs = np.arange(1, n + 1)  # data points at 1..N

    plt.figure(figsize=(8, 5))
    plt.plot(epochs, val_f1_history,
             marker="o", markersize=3, linewidth=1.6,
             linestyle="-", color="blue", label="Validation F1")
    ...
    # show ticks at 0,10,... up to N (so if N=100 you'll get 0..100)
    tick_step = 10
    xticks = np.arange(0, n + 1, tick_step)
    plt.xticks(xticks)
    plt.xlim(0, n)            # so the 0 tick appears on the left
...
if __name__ == "__main__":
    ...
    # ---------- 2.1: embedding + logistic model ----------
    embedding_dim = 100
    embedding = nn.Embedding(len(index_map), embedding_dim, padding_idx=0)
    model = PooledLogisticRegression(embedding)

    # ---------- 2.2: optimizer (SGD from earlier) ----------
    optimizer = assign_optimizer(model, lr=0.001)

    # ---------- 2.3: single-batch forward & loss (smoke test) ----------
    y_pred = forward_pass(model, batch)  # shape (N,)
    loss = bce_loss(y, y_pred)  # scalar tensor

    # ---------- 2.4: single-batch F1 ----------
    score = f1_score(y, y_pred, threshold=0.5)  # tensor([f1])

    # ---------- 2.5: train logistic model & plot ----------
    n_epochs = 100
    # (Recreate clean model/optimizer before full training run)
    embedding = nn.Embedding(len(index_map), embedding_dim, padding_idx=0)
    model = PooledLogisticRegression(embedding)
    optimizer = assign_optimizer(model, lr=0.01)

    scores = train_loop(model, train_loader, valid_loader, optimizer, n_epochs=n_epochs)
    plot_training_curve(scores, "training_curve.png")
